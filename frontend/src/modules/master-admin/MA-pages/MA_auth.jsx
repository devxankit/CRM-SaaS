...
import { FaEye, FaEyeSlash, FaUser, FaLock, FaArrowRight, FaCrown } from 'react-icons/fa'
...
const MA_auth = () => {
  ...
  useEffect(() => {
    if (masterAdminAuthService.isAuthenticated()) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await masterAdminAuthService.login(formData.email, formData.password)
      
      if (response.success) {
        // Store master admin data
        masterAdminAuthService.storeMasterAdminData({
          ...response.data.masterAdmin,
          loginTime: new Date().toISOString()
        })
        
        // Show success toast
        toast.login(`Welcome back, ${response.data.masterAdmin.name}!`, {
      ...
      >
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 p-8 relative overflow-hidden">
          ...
          >
            ...
            <p className="text-gray-600 text-sm">Sign in to your master admin account</p>
          </motion.div>
