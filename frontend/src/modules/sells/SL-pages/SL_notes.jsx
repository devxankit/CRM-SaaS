          ...
          >
            ...
            <div className="flex items-center space-x-4 mb-8 p-4 bg-gray-50 rounded-xl">
              <div className="w-16 h-16 bg-gradient-to-br from-teal-500 to-teal-600 rounded-full flex items-center justify-center">
                <span className="text-xl font-bold text-white">{client.name[0]}</span>
              </div>
              <div>
            ...
            {/* Add Note Section */}
            <div className="mb-8">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">Add New Note</h3>
              <div className="space-y-4">
                <textarea
                  value={newNote}
                  onChange={(e) => setNewNote(e.target.value)}
                  placeholder="Write a note about this client..."
                  className="w-full p-4 border border-gray-200 rounded-xl bg-white text-gray-900 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all duration-200 resize-none"
                  rows={4}
                />
                <button
                  onClick={handleAddNote}
                  disabled={isLoading || !newNote.trim()}
                  className="bg-gradient-to-r from-teal-500 to-teal-600 text-white py-3 px-6 rounded-xl font-semibold flex items-center justify-center space-x-2 hover:from-teal-600 hover:to-teal-700 transition-all duration-200 disabled:opacity-50"
                >
                  {isLoading ? (
                    <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
                  ) : (
            ...
            {/* Notes List */}
            <div>
              <h3 className="text-xl font-semibold text-gray-900 mb-6">Previous Notes</h3>
              
              {notes.length === 0 ? (
                <div className="text-center py-12">
                  <FiEdit3 className="text-gray-400 text-6xl mx-auto mb-4" />
                      ...
                      {isEditing && editingNoteId === note.id ? (
                        <div className="space-y-4">
                          <textarea
                            value={editingContent}
                            onChange={(e) => setEditingContent(e.target.value)}
                            className="w-full p-4 border border-gray-200 rounded-xl bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all duration-200 resize-none"
                            rows={3}
                          />
                          <div className="flex space-x-3">
                            <button
                              onClick={handleSaveEdit}
                              disabled={isLoading}
                              className="bg-teal-500 text-white py-2 px-4 rounded-lg font-medium hover:bg-teal-600 transition-colors duration-200 disabled:opacity-50"
                            >
                              Save
                            </button>
                            <button
                              onClick={handleCancelEdit}
                              className="bg-gray-200 text-gray-700 py-2 px-4 rounded-lg font-medium hover:bg-gray-300 transition-colors duration-200"
                            >
                              Cancel
                            </button>
                          </div>
                        ...
                        <div>
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex-1">
                              <p className="text-gray-900 leading-relaxed text-lg">{note.content}</p>
                            </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                              <span className={`px-3 py-1 rounded-full text-sm font-medium ${getNoteTypeColor(note.type)}`}>
                                {note.type}
                              </span>
                            </div>
                            <div className="flex items-center space-x-2 text-gray-500">
                              <FiClock className="text-sm" />
                              <span className="text-sm">{formatTimestamp(note.timestamp)}</span>
                            </div>
                          </div>
                        </div>
                      )}
